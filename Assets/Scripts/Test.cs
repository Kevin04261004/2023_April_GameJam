...
public class Test : MonoBehaviour
{
    ...
    void Update_GM()
    {
        ...
        shotable = true;
        for (int i = 0; i < BallGroup.childCount; i++)
            if (BallGroup.GetChild(i).GetComponent<Test>().isMoving) shotable = false;
    ...
    void FixedUpdate_GM()
    {
        ...
        {
            timerCount = 0;
            BallGroup.GetChild(launchIndex++).GetComponent<Test>().Launch(gap);
    ...
    void Start_BALL() => GM = GameObject.FindWithTag("GameManager").GetComponent<Test>();
